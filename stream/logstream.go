...
func InitLogStream(operatorURI, operatorName string, batchSize, nWorkers, skipToIndex int, verbose bool) (*LogStream, error) {
	...
	if err != nil {
		klog.Errorf("could not get CT client: %v", err)
		return nil, err
	}
	ls := &LogStream{
		LogClient:     logClient,
...
func (ls *LogStream) Stream(ctx context.Context, toAgg chan<- *certificate.Batch) {
	...
	if err != nil {
		klog.Errorf("could not get STH: %v", err)
		return
	}
	// Step 3 in batches
	...
	wg.Wait()
	klog.Infof("%s workers have stopped", ls.String())
}
...
func (ls *LogStream) updateSTH(ctx context.Context) error {
	...
	fn := func() error {
		...
		if err != nil {
			attempts += 1
			klog.Infof("%s [attempts=%d] could not get STH: %v",
				ls.String(),
				attempts,
				err,
			)
	...
	if err := ls.LogClient.VerifySTHSignature(*head); err != nil {
		klog.Infof("could not verify STH: %v", err)
		return err
	}
	...
	klog.Infof("%s STH updated: start=%d end=%d",
		ls.String(),
		ls.IndexStart,
		ls.IndexEnd,
	)
	...
	go func() {
		...
		for {
			...
			if start >= end {
				...
				err := ls.updateSTH(ctx)
				if err != nil {
					klog.Errorf("%s could not update STH: %v", ls.String(), err)
					return
				}
				klog.Infof("%s STH updated: start=%d end=%d", ls.String(), start, end)
				end = ls.IndexEnd
			}
			...
			case batches <- batch:
			case <-ctx.Done():
				klog.Infof("%s will stop sending batches", ls.String())
				return
			}
			klog.Infof("%s batch created: start=%d end=%d",
				ls.String(),
				start,
				batchEnd,
			)
	...
	for batch := range batches {
		for batch.Start <= batch.End {
			...
			fn := func() error {
				...
				if err != nil {
					klog.Errorf("%s could not GetRawEntries: %v", ls.String(), err)
					return err
				}
				return nil
			}
			...
			if err != nil {
				klog.Infof("%s backoff for GetRawEntries failed: %v", ls.String(), err)
				return
			}
			if ctx.Err() != nil {
			...
			for i, entry := range entriesResp.Entries {
				...
				if x509.IsFatal(err) {
					klog.Errorf("%s could not parse x509 leaf cert: %v", ls.String(), err)
					continue
				}
